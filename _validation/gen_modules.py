...
def gen_child_route(mid, child_table, fk):
    # Read-only child sub-collection (detail panel). GET <route>/[id]/<subPath> -> child rows by FK, masked.
    return ('// GENERATED by _validation/gen_modules.py — DO NOT EDIT.\n'
            ...
            f'  const guard = await requireStaffScope(request, {json.dumps(mid)}, "read");\n'
            '  if (!guard.ok) return NextResponse.json(guard.body, { status: guard.status });\n'
            '  const { id } = await ctx.params;\n'
            f'  const data = await listChildRecords({json.dumps(child_table)}, {json.dumps(fk)}, id, guard.actor);\n'
            f'  return NextResponse.json(ok(data, meta(guard.requestId, {json.dumps(mid)})));\n'
            '}\n')
...
def generate_module(mid, route, table, status_col=None, scope="staff", spec_module=None, exclude=None):
    ...
    for p in panels:
        ...
        (cdir/"route.ts").write_text(gen_child_route(mid, p["table"], p["fk"]), encoding="utf-8")
