...
import _ui_fields
NEEDS_BUILDER = {}   # table -> True when a required payload/json column means the generic form is insufficient
...
STAFF_CUSTOM_NAV = [("staff/reports/exports", "Sensitive Exports"), ("staff/admin/settings/changes", "Setting Changes"), ("staff/security-audit/integrity", "Audit Integrity"), ("staff/exams/materials/upload", "Upload Materials")]
...
STATUS_ACTION = {
 ...
 "under_review":("start_review","blue"),"resolved":("resolve","blue"),"accepted":("accept","blue"),
 "retired":("retire","light"),"reopened":("reopen","light"),"escalated":("escalate","light"),"applied":("apply","blue"),
}
...
def create_fields(table, status_col=None, extra_hidden=frozenset()):
    ...
    for f in fields:
        if f.get("type") == "reference" and f.get("refTable"):
            REF_TARGETS.add(f["refTable"])
    return fields

def display_columns(table, status_col):
    return _ui_fields.display_columns(table, MODEL, status_col, COLUMN_OVERRIDES)
...
def emit_lookup_labels():
    """table -> human label column for the generic /api/staff/lookup/[table] reference-picker endpoint.
    Covers every FK-target table in the model, so ANY reference renders as a business dropdown."""
    targets = {}
    for _tbl, t in MODEL.items():
        for c in t.get("columns", []):
            ref = _ui_fields._ref_table(c)
            if ref and ref in MODEL:
                targets[ref] = _ui_fields.label_column(ref, MODEL)
    lines = ["// GENERATED by gen_ui.emit_lookup_labels — DO NOT EDIT.",
    ...
    for tbl in sorted(targets):
        lines.append(f"  {json.dumps(tbl)}: {json.dumps(targets[tbl])},")
    lines += ["};", ""]
    write(APP / "server" / "lookups" / "labelColumns.generated.ts", "\n".join(lines))
...
def emit_school_scope():
    ...
    for tbl in sorted(targets):
        ...
        scope[tbl] = {"kind": "none"}
    lines = ["// GENERATED by gen_ui.emit_school_scope — DO NOT EDIT.",
    ...
    lines += ["};", ""]
    write(APP / "server" / "lookups" / "schoolScope.generated.ts", "\n".join(lines))
...
if __name__ == "__main__":
    ...
    _Path("_validation/reports/ui_page_tables.json").write_text(json.dumps(PAGE_TABLES, indent=0), encoding="utf-8")
    builders = sorted(t for t, v in NEEDS_BUILDER.items() if v)
    print("pages generated:", count, "+ navLinks.ts + lookup labels")
    if builders:
        print("NEEDS WORKFLOW BUILDER (required payload/json — generic form insufficient):", ", ".join(builders))
