...
# human-readable field labels for validation messages (one source). Default: spaced + sentence-case.
_LABELS = {"html_content": "Email body", "campaign_code": "Code"}


def nice(fn):
    s = _LABELS.get(fn) or fn.replace("_", " ")
...
def main():
    ...
    for name in sorted(entities):
        ...
        for f in e["fields"]:
            ...
            if mdig:
                n = mdig.group(1)
                checks.append(f'  if (input.{fn} !== undefined && String(input.{fn}) !== "" && !/^[0-9]{{{n}}}$/.test(String(input.{fn}))) errors.push({{ field: "{fn}", message: "{nice(fn)} must be {n} digits" }});')
            if fn == "status":
                ...
                if allowed:
                    ...
                    checks.append(f'  if (input.status !== undefined && ![{vals}].includes(String(input.status))) errors.push({{ field: "status", message: "{nice("status")} is not a valid status code" }});')
            elif f["type"] == "enum" and f.get("enum_values"):
                vals = ", ".join(f'"{v}"' for v in f["enum_values"])
                checks.append(f'  if (input.{fn} !== undefined && ![{vals}].includes(String(input.{fn}))) errors.push({{ field: "{fn}", message: "{nice(fn)} is not a valid value" }});')
