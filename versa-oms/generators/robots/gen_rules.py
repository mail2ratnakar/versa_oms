...
def main():
    ...
    # entity -> declared status values (from its workflow's states)
    ent_states = {}
    for wf, w in catalog["workflows"].items():
        e = entity_for_workflow(wf, entities)
    ...
    for name in sorted(entities):
        ...
        for f in e["fields"]:
            ...
            if fn == "status" and name in ent_states:
                vals = ", ".join(f'"{v}"' for v in sorted(ent_states[name]))
                checks.append(f'  if (input.status !== undefined && ![{vals}].includes(String(input.status))) errors.push({{ field: "status", message: "status is not a declared state" }});')
