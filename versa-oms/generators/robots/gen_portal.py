...
loadKpis();"""
    ...
    if shape == "manage":
        ...
        compose_btns = (f'<button class="btn secondary" onclick="openPreview()">{icon("eye", 16)} Preview</button>'
    ...
    body = (f'{stepper}{toolbar}{rcvbanner}<section><div class="head"><div><h3>{label(entity)}</h3></div>{new_btn}</div>{bulkbar}'
            f'<div class="tablewrap"><table><thead><tr>{thead}</tr></thead><tbody id="rows"></tbody></table></div>{pager}</section>{modal}{detail}{more_modal}{pv_modal}')
...
def portal_page(j, nav, symbols, body, script, portal, shell):
    ...
    tz_script = (f"const TZ={{mode:{json.dumps(tz.get('mode','auto'))},zone:{json.dumps(tz.get('zone','Asia/Kolkata'))}}};"
                 ...
                 "function openPreview(){const pv=document.getElementById('pv');if(!pv)return;const sub=document.getElementById('pvsub');if(sub)sub.textContent='Subject: '+(pvFill(pvSubject())||'(none)');const f=document.getElementById('pvif');if(f)f.srcdoc=pvFill(pvBody())||'<p style=\"font-family:system-ui;color:#999;padding:24px\">Empty body — paste HTML or text in the form, then Preview.</p>';setPv('d');pv.classList.add('open');}"
