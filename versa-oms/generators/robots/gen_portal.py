...
def form_fields(entity, ents, hide):
    ...
    for f in ents[entity]["fields"]:
        ...
        if fn in REF:
            ...
            div = f'<div class="field"><label>{label(fn)}</label><input class="input" name="{fn}" maxlength="6" inputmode="numeric" oninput="pinLookup(this)" placeholder="6-digit — fills city/state"></div>'
            ...
            if "email" in nl:
                ...
                at = 'data-kind="text"'
            div = f'<div class="field"><label>{label(fn)}</label><input class="input" name="{fn}" {at}></div>'
