...
def portal_page(j, nav, symbols, body, script, portal, shell):
    ...
    tz_script = (f"const TZ={{mode:{json.dumps(tz.get('mode','auto'))},zone:{json.dumps(tz.get('zone','Asia/Kolkata'))}}};"
                 ...
                 "async function pinLookup(el){const v=el.value.replace(/[^0-9]/g,'').slice(0,6);el.value=v;if(v.length!==6)return;try{const r=await fetch('https://api.postalpincode.in/pincode/'+v);const j=await r.json();const po=j&&j[0]&&j[0].PostOffice&&j[0].PostOffice[0];if(!po)return;const sc=el.closest('.modal')||el.closest('section')||document;const c=sc.querySelector('[name=city]');const s=sc.querySelector('[name=state]');if(c)c.value=po.District||po.Name||'';if(s){if(s.tagName==='SELECT'){for(const o of s.options)if(o.value.toLowerCase()===String(po.State||'').toLowerCase())s.value=o.value;}else s.value=po.State||'';}}catch(e){}}")
