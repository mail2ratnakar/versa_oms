...
def form_fields(entity, ents, hide):
    ...
    for f in ents[entity]["fields"]:
        fn = f["name"]
        if fn in ("id", "created_at", "updated_at", "status") or fn in hide or "system" in f.get("rule", ""):
            continue  # never expose computed/system fields as form inputs
        if fn in REF:
            out.append(f'<div class="field"><label>{label(fn)}</label><select class="select" name="{fn}"><option value="">Select…</option>' + "".join(f"<option>{v}</option>" for v in REF[fn]) + "</select></div>")
        elif fn == "pincode":
            out.append(f'<div class="field"><label>{label(fn)}</label><input class="input" name="{fn}" maxlength="6" inputmode="numeric" oninput="pinLookup(this)" placeholder="6-digit — fills city/state"></div>')
            ...
            if "email" in nl:
                ...
                at = 'data-kind="text"'
            out.append(f'<div class="field"><label>{label(fn)}</label><input class="input" name="{fn}" {at}></div>')
    return "\n            ".join(out)
...
loadKpis();"""
    ...
    if shape == "manage":
        ...
        modal = (f'<div id="m" class="modalbg" onclick="if(event.target===this)closeModal()"><div class="modal" style="max-height:85vh;overflow:auto">'
                 f'<div class="between"><h3 id="mtitle" style="margin:0">New {entlabel}</h3><button class="btn ghost iconbtn" onclick="closeModal()">{icon("x", 16)}</button></div>'
                 f'<div class="grid two" id="mform" style="margin-top:14px">{fields}</div>'
                 f'<div id="msys" class="detailgrid" style="margin-top:6px"></div>'
                 f'<div class="flex" style="margin-top:16px"><button class="btn primary" onclick="save()">{icon("check", 16)} Save</button>'
...
function openMore(x){{const b=document.getElementById('moreBody');b.replaceChildren();document.getElementById('moreTitle').textContent='Actions — '+(x.name||x.id);
  for(const tr of ALL_TRANS){{const active=tr.staff&&(tr.from===x.status||tr.from==='any');const row=document.createElement('div');row.className='actrow'+(active?'':' muted');
    ...
    const right=document.createElement('div');if(active){{const btn=document.createElement('button');btn.className='btn secondary tiny';btn.textContent='Do';btn.onclick=()=>{{act(x.id,tr.action);closeMore();}};right.appendChild(btn);}}else{{const tnote=document.createElement('span');tnote.className='muted tiny';tnote.textContent=tr.staff?('needs '+tr.from):'school-driven';right.appendChild(tnote);}}
