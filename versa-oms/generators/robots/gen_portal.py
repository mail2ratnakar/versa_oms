...
function render(){{const rows=filtered();const pages=Math.max(1,Math.ceil(rows.length/PSIZE));if(PAGE>pages)PAGE=pages;if(PAGE<1)PAGE=1;const pr=rows.slice((PAGE-1)*PSIZE,(PAGE-1)*PSIZE+PSIZE);
  ...
  if(!pr.length){{const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=SPAN;td.className='muted';td.textContent='No matches.';tr.appendChild(td);tb.appendChild(tr);}}
    ...
    if(ACTS.length||TOGGLES.length){{const td=document.createElement('td');const avail=ACTS.filter(a=>(TRANS[a]||['any']).some(fr=>fr==='any'||fr===x.status));for(const a of avail){{const b=document.createElement('button');b.className='btn secondary tiny';b.style.marginRight='6px';b.textContent=a.replace(/_/g,' ');b.addEventListener('click',(ev)=>{{ev.stopPropagation();act(x.id,a);}});td.appendChild(b);}}for(const tg of TOGGLES){{const on=!!x[tg.field];const b=document.createElement('button');b.className='btn ghost tiny';b.style.marginRight='6px';b.textContent=on?tg.off:tg.on;b.addEventListener('click',(ev)=>{{ev.stopPropagation();toggleField(x.id,tg.field,!on);}});td.appendChild(b);}}tr.appendChild(td);}}
    tb.appendChild(tr);}}
