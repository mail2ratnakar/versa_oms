...
loadKpis();"""
    ...
    if shape == "register":
        ...
        script = ("async function register(){const _ve=validate(document.querySelector('.card'));if(_ve){document.getElementById('msg').textContent=_ve;return;}const input={};document.querySelectorAll('.card [name]').forEach(el=>{if(el.value)input[el.name]=el.value;});"
                  ...
                  "const j=await r.json();if(j.ok&&j.data&&j.data.id){try{await fetch('/api/schools/'+j.data.id+'/submit_registration',{method:'POST'});}catch(e){}}document.getElementById('msg').textContent=j.ok?('Registered — '+j.data.name+' is now pending approval.'):errText(j);}"
    ...
    tabs = j.get("tabs", [])
    tabs_html = ('<div class="tabs" style="margin-bottom:14px">' + "".join('<button class="tab' + (' active' if _i == 0 else '') + '" onclick="setTab(' + str(_i) + ')">' + _t["label"] + '</button>' for _i, _t in enumerate(tabs)) + '</div>') if tabs else ""
...
function render(){{const rows=filtered();const pages=Math.max(1,Math.ceil(rows.length/PSIZE));if(PAGE>pages)PAGE=pages;if(PAGE<1)PAGE=1;const pr=rows.slice((PAGE-1)*PSIZE,(PAGE-1)*PSIZE+PSIZE);
  ...
  if(!pr.length){{const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=SPAN;td.className='muted';td.textContent='No matches.';tr.appendChild(td);tb.appendChild(tr);}}
    ...
    if(DOWNLOAD){{const td=document.createElement('td');const b=document.createElement('button');b.className='btn secondary tiny';b.textContent='Download';b.addEventListener('click',(ev)=>{{ev.stopPropagation();window.open('/api/'+ENTITY+'/'+x.id+'/render?code='+encodeURIComponent(x.verification_code||''),'_blank');}});td.appendChild(b);tr.appendChild(td);}}
...
function setTab(i){{TAB=i;document.querySelectorAll('.tabs .tab').forEach(function(e,k){{e.classList.toggle('active',k===i);}});PAGE=1;render();}}
...
PORTALS = [
    {"spec": "versa-oms/spec/public_journeys.json", "dir": "versa-oms/spec/derived/public", "brand": "Versa Olympiads", "navlabel": "", "scoped": False, "standalone": True},
