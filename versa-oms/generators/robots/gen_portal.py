...
loadKpis();"""
    ...
    if shape == "import":
        ...
        body = (f'<div class="stepper">{ws}</div><section class="card">'
                ...
                f'<div class="flex" style="margin-top:14px"><a class="btn primary" href="OJ-O2.html">{icon("arrow", 16)} School directory</a> <button class="btn ghost" onclick="location.reload()">New import</button></div></div></section>')
        script = (
            "let UP=null;\n"
...
async function openEdit(x){{EDIT_ID=x.id;document.getElementById('mtitle').textContent='Edit '+ENT;await populateFk();
  ...
  const sys=document.getElementById('msys');sys.replaceChildren();for(const k of SYS){{const r=document.createElement('div');r.className='drow';const a=document.createElement('span');a.className='dk';a.textContent=k.replace(/_/g,' ');const v=document.createElement('span');v.className='dv';v.textContent=(x[k]==null||x[k]==='')?'—':x[k];r.append(a,v);sys.appendChild(r);}}
