...
loadKpis();"""
    ...
    if shape == "builder":
        ...
        gjs_modal = ('<div id="gjsModal" class="gjsmodal">'
                     '<div class="gjsbar"><b>Design email</b><div class="brow">'
                     '<button class="btn primary" onclick="gjsDone()">Apply</button>'
                     '<button class="btn ghost" onclick="gjsCancel()">Cancel</button></div></div>'
                     '<div id="gjs"></div></div>')
        body = (f'<a href="OJ-O3.html" class="btn ghost tiny" style="margin-bottom:10px">{icon("arrow", 14)} Back to campaigns</a>'
                ...
                '<button id="tabHtml" class="btn ghost tiny" onclick="bTab(\'html\')">HTML</button>'
                f'<button class="btn ghost tiny" onclick="openGjs()">{icon("layout", 13)} Design</button></div></div>'
        ...
        script = (f"const ENTITY={json.dumps(entity)};const TEMPLATES={json.dumps(EMAIL_TEMPLATES)};const LISTPAGE='OJ-O3.html';const SCHEDULE_FIELD='scheduled_at';\n"
                  ...
                  "let GJS=null,GJS_LOADED=false;"
                  "function loadScript(src){return new Promise(function(res,rej){const s=document.createElement('script');s.src=src;s.onload=res;s.onerror=rej;document.head.appendChild(s);});}"
                  "async function ensureGjs(){if(GJS_LOADED)return;const l=document.createElement('link');l.rel='stylesheet';l.href='/vendor/grapes.min.css';document.head.appendChild(l);await loadScript('/vendor/grapes.min.js');await loadScript('/vendor/grapesjs-preset-newsletter.min.js');GJS_LOADED=true;}"
                  "async function openGjs(){try{await ensureGjs();}catch(e){toast('Could not load the designer','err');return;}document.getElementById('gjsModal').classList.add('open');if(!GJS){GJS=grapesjs.init({container:'#gjs',height:'100%',fromElement:false,storageManager:false,plugins:['grapesjs-preset-newsletter'],assetManager:{uploadFile:async function(ev){const files=ev.dataTransfer?ev.dataTransfer.files:ev.target.files;for(const f of files){try{const a=await doUpload(f);GJS.AssetManager.add(a.url);}catch(e){toast('Upload failed','err');}}}}});}GJS.setComponents(bBody()||'<p>Start designing your email…</p>');}"
                  "function gjsDone(){if(GJS){let html;try{html=GJS.runCommand('gjs-get-inlined-html');}catch(e){html=GJS.getHtml();}setBody(html);toast('Design applied');}document.getElementById('gjsModal').classList.remove('open');}"
                  "function gjsCancel(){document.getElementById('gjsModal').classList.remove('open');}"
