...
- **P4.6 After EVERY implementation, design unit tests per unit, run each, and fix bugs as they surface — before declaring done.** Enumerate the units of the feature (each filter, each pill, pagination, empty state, the record actually rendering past page 1, scope, masking) and write a test for each; a feature that "worked in one curl" is not verified. Counts being right ≠ records being visible. (Learned: the CRM toolbar shipped with correct facet counts but page-2 records were unreachable — no pagination test existed.)
