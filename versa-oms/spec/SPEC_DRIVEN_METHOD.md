...
|---|---|---|
| CRM custom routes + `server/crm/leadService.ts` (incl. CHAIN-001 convert effects) | `feature_effects/.../FEATURE_EFFECTS.md` (FX endpoints + server effects) | `gen_actions.py` (emit custom action routes + service stubs) |
...
DONE: `server/lib/transitionEffects.ts` is now generated by `gen_effects.py` from
`spec/effects/chains.json` (CHAIN-002 transition-triggered effects). CHAIN-001's effects
still live in `leadService.convertLead` because convert is a custom action, not a kernel
transition — they move under generation when `gen_actions.py` exists.
