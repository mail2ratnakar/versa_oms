...
| certificate_ops | SF | 9→5 | no real generation (number/file/hash/QR are **dead helpers**); reissue + impact handling; public verify page is a static skeleton |
...
5. **Secure file handling unwired.** `signedUrl.ts` / `createSignedDownloadUrl` exists but **0 routes call it**. No `downloadAction` on exam materials, certificates, reports, exports. Files are bare uuid refs.
...
8. **Generated create forms drift from server Zod schemas** → broken creates: omr core, courier core (omits `school_id`/`participation_id`/`courier_direction`), certificates (user types unique number), evaluation/results forms omit required FKs → 422.
