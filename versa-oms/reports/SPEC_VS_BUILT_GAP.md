...
| evaluation_ops | SF→PF* | 8→5 | *FR-OMR-SCORING-0008 (2026-06-25): scoring engine WIRED — score-batch scoring run reads responses + approved answer key → persists evaluation_candidate_scores (correct/wrong/blank, neg-marking). REMAINING: OMR raw-scan import/parsing (importConfig + parser_confidence), score→result handoff into candidate_results, PII masking on responses |
