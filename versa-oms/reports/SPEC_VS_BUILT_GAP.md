...
| evaluation_ops | SF→PF* | 8→6 | *FR-OMR-SCORING-0008 + FR-OMR-IMPORT-0010 (2026-06-25): **response import** (CSV → evaluation_candidate_responses) + **scoring** (responses + approved key → evaluation_candidate_scores) both WIRED; score→result handoff done (FR-0009). REMAINING: real OMR raw-scan parsing (binary + parser_confidence), PII masking on responses, per-candidate school resolution |
| results_ops | SF→PF* | 8→6 | *FR-RESULTS-RANKING-0006 + FR-RESULT-HANDOFF-0009 (2026-06-25): server-side ranking (national/grade/subject) + cert-eligibility snapshot + published-result immutability + **score→result handoff** (generate derives candidate_results from the linked scored batch); migration 0022 fixed candidate_results UNIQUEs. REMAINING: correction-versioning (new batch), finance-hold, tie-break/threshold policy config, per-candidate grade |
| results | SF/ORPH | 3→0 | build follows `results_ops`; this spec orphaned — decide deprecate/reconcile |
| certificate_ops | SF→LF* | 9→7 | *FR-CERT-GENERATION-0004 + FR-CERT-PDF-0005 + FR-CERT-SEAL-0011 (2026-06-25): server-gen number/code, publish→public_verification, revoke reflected, real verify page, PDF gen + private storage + secure signed download with QR, **HMAC digital seal + tamper-evident integrity check**. REMAINING: asymmetric/public-key signatures (offline verify), reissue/impact handling, core_certificates from-state guards, olympiad/award result-join |
