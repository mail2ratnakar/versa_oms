...
async function handle(req: any, res: any) {
  ...
  if (path === "/workflowbuilder" || path === "/workflowbuilder.html") { try { const b = await readFile("spec/derived/workflowbuilder.html"); res.writeHead(200, { "content-type": "text/html" }); res.end(b); } catch { res.writeHead(404); res.end("run: python versa-oms/generators/robots/gen_workflowbuilder.py"); } return; }
  if (path === "/appmap" || path === "/appmap.html") { try { const b = await readFile("spec/derived/appmap.html"); res.writeHead(200, { "content-type": "text/html" }); res.end(b); } catch { res.writeHead(404); res.end("run: python versa-oms/generators/robots/gen_appmap.py"); } return; }
