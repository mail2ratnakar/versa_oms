...
async function handle(req: any, res: any) {
  ...
  { const m = path.match(/^\/api\/certificates\/([^/]+)\/render$/); if (m && req.method === "GET") { const html = await renderCertificate(m[1]); res.writeHead(200, { "content-type": "text/html" }); res.end(html); return; } }
