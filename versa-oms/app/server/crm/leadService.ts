...
export async function addInteraction(actor: Actor, leadId: string, payload: Record<string, unknown>) {
  ...
  if (String(payload["next_followup_at"] ?? "").trim()) {
    ...
    const { raiseNotificationEvent } = await import("@/server/notifications/fanout");
    await raiseNotificationEvent(supabase, { event_code: "crm_followup_due", event_idempotency_key: "crm_followup_due:lead_owner:" + newId, source_module: "school_crm", source_entity: "school_lead_interactions", source_entity_id: newId, event_payload: { message: "A CRM follow-up is due.", due: row["next_followup_at"] }, recipient_resolver: "lead_owner" }, actor);
    await raiseNotificationEvent(supabase, { event_code: "crm_followup_due", event_idempotency_key: "crm_followup_due:sales_manager:" + newId, source_module: "school_crm", source_entity: "school_lead_interactions", source_entity_id: newId, event_payload: { message: "A team CRM follow-up is due.", due: row["next_followup_at"] }, recipient_resolver: "sales_manager" }, actor);
