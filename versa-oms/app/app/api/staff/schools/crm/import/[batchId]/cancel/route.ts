...
import { cancelImport } from "@/server/crm/leadService";
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ batchId: string }> }) {
  ...
  try {
    return NextResponse.json(ok(await cancelImport(guard.actor, batchId, String(body.reason ?? "")), meta(guard.requestId, "school_crm")));
  } catch (e) {
